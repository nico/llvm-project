...
public:
  ...
  size_t getLength() override { return ImmutableStream.getLength(); }

  Error writeBytes(size_t Offset, ArrayRef<uint8_t> Buffer) override {
    if (Buffer.empty())
      return Error::success();
...
public:
  ...
  void insert(size_t Offset, ArrayRef<uint8_t> Bytes) {
  ...
  Error writeBytes(size_t Offset, ArrayRef<uint8_t> Buffer) override {
    ...
    size_t RequiredSize = Offset + Buffer.size();
...
public:
  ...
  size_t getLength() override { return Impl.getLength(); }

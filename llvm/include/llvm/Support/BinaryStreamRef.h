...
protected:
  ...
  BinaryStreamRefBase(std::shared_ptr<StreamType> SharedImpl, size_t Offset,
                      Optional<size_t> Length)
      : SharedImpl(SharedImpl), BorrowedImpl(SharedImpl.get()),
        ViewOffset(Offset), Length(Length) {}
  BinaryStreamRefBase(StreamType &BorrowedImpl, size_t Offset,
                      Optional<size_t> Length)
...
public:
  ...
  /// too.
  RefType drop_front(size_t N) const {
    if (!BorrowedImpl)
      return RefType();
  ...
  RefType drop_back(size_t N) const {
    if (!BorrowedImpl)
      return RefType();
  ...
  /// removed.
  RefType drop_symmetric(size_t N) const {
  ...
  RefType slice(size_t Offset, size_t Len) const {
...
protected:
  Error checkOffsetForRead(size_t Offset, size_t DataSize) const {
    if (Offset > getLength())
...
class BinaryStreamRef
  ...
  BinaryStreamRef(std::shared_ptr<BinaryStream> Impl, size_t ViewOffset,
                  Optional<size_t> Length)
...
public:
  ...
  BinaryStreamRef(BinaryStream &Stream, size_t Offset,
                  Optional<size_t> Length);
  ...
  BinaryStreamRef(BinaryStreamRef &S, size_t Offset,
                  size_t Length) = delete;
...
struct BinarySubstreamRef {
  size_t Offset = 0;        // Offset in the parent stream
  BinaryStreamRef StreamData; // Stream Data

  BinarySubstreamRef slice(size_t Off, size_t Size) const {
    BinaryStreamRef SubSub = StreamData.slice(Off, Size);
    return {Off + Offset, SubSub};
  }
  BinarySubstreamRef drop_front(size_t N) const {
    return slice(N, size() - N);
  }
  BinarySubstreamRef keep_front(size_t N) const { return slice(0, N); }

  std::pair<BinarySubstreamRef, BinarySubstreamRef>
  split(size_t Off) const {
    return std::make_pair(keep_front(Off), drop_front(Off));
  }

  size_t size() const { return StreamData.getLength(); }
...
class WritableBinaryStreamRef
  ...
  WritableBinaryStreamRef(std::shared_ptr<WritableBinaryStream> Impl,
                          size_t ViewOffset, Optional<size_t> Length)
      : BinaryStreamRefBase(Impl, ViewOffset, Length) {}

  Error checkOffsetForWrite(size_t Offset, size_t DataSize) const {
    if (!(BorrowedImpl->getFlags() & BSF_Append))
...
public:
  ...
  WritableBinaryStreamRef(WritableBinaryStream &Stream, size_t Offset,
                          Optional<size_t> Length);
  ...
  WritableBinaryStreamRef(WritableBinaryStreamRef &S, size_t Offset,
                          size_t Length) = delete;
  ...
  Error writeBytes(size_t Offset, ArrayRef<uint8_t> Data) const;

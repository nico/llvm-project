...
private:
  ...
  template <class RelTyA, class RelTyB>
  bool constantEq(const InputSection *a, Relocs<RelTyA> relsA,
                  const InputSection *b, Relocs<RelTyB> relsB);
...
uint64_t ICF<ELFT>::constantRelocHash(const InputSection *sec,
  ...
  for (const RelTy &rel : rels) {
    ...
    if (s->isDefined()) {
      ...
      if (SectionBase *targetSec = d->section) {
        if (targetSec->kind() == SectionBase::Regular) {
          targets.push_back(static_cast<InputSection *>(targetSec));
  ...
  const_cast<InputSection *>(sec)->setIcfTargets(targets);
  return hash;
}
...
template <class ELFT>
template <class RelTyA, class RelTyB>
bool ICF<ELFT>::constantEq(const InputSection *secA, Relocs<RelTyA> ra,
                           const InputSection *secB, Relocs<RelTyB> rb) {
  if (ra.size() != rb.size())
    return false;
  const Symbol *const *symbolsA = secA->file->getSymbols().data();
  const Symbol *const *symbolsB = secB->file->getSymbols().data();
  const bool isMips64EL = ctx.arg.isMips64EL;
  auto rai = ra.begin(), rae = ra.end();
  auto rbi = rb.begin();
  for (; rai != rae; ++rai, ++rbi) {
...
bool ICF<ELFT>::equalsConstant(const InputSection *a,
  ...
  if (ra.areRelocsCrel()) {
    ...
    if (rb.areRelocsRel())
      return constantEq(a, ra.crels, b, rb.rels);
    return constantEq(a, ra.crels, b, rb.relas);
  }
  if (ra.areRelocsRel()) {
    if (rb.areRelocsCrel())
      return constantEq(a, ra.rels, b, rb.crels);
    if (rb.areRelocsRel())
      return constantEq(a, ra.rels, b, rb.rels);
    return constantEq(a, ra.rels, b, rb.relas);
  }
  if (rb.areRelocsCrel())
    return constantEq(a, ra.relas, b, rb.crels);
  if (rb.areRelocsRel())
    return constantEq(a, ra.relas, b, rb.rels);
  return constantEq(a, ra.relas, b, rb.relas);
}
...
bool ICF<ELFT>::equalsVariable(const InputSection *a, const InputSection *b) {
  ArrayRef<InputSection *> ta = a->icfTargets();
  ArrayRef<InputSection *> tb = b->icfTargets();
  assert(ta.size() == tb.size());
  for (size_t i = 0, n = ta.size(); i < n; ++i) {
    const InputSection *x = ta[i];
    const InputSection *y = tb[i];
...
std::vector<size_t> ICF<ELFT>::rangeBlocks() const {
  ...
  size_t perBlock = std::max<size_t>(total / numBlocks, 64);
...
template <class ELFT> void ICF<ELFT>::run() {
...

  {
    ...
    for (unsigned cnt = 0; cnt != 2; ++cnt) {
      parallelForEach(sections, [&](InputSection *s) {
        ArrayRef<InputSection *> targets = s->icfTargets();
        if (targets.empty())
          return;
        uint32_t hash = s->eqClass[cnt % 2];
        for (const InputSection *target : targets)
          hash += target->eqClass[cnt % 2];
        // Set MSB to 1 to avoid collisions with unique IDs.
